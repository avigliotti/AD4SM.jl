...
struct C1D{N,M}
  ...
  Nx::NTuple{N,Array{Float64,1}}
  wgt::NTuple{N,Float64}
  V::Real
  mat::M
end
struct C2D{N,M}
...
CElems  = Union{C2D, C3D, CAS, C2DP, C3DP}
CartEls = Union{C2D, C3D}
# copy
copy(elem::C1D{M}) where M = C2D{M}(copy(elem.nodes), elem.Nx, elem.wgt, elem.V, elem.mat)
...
# clone
function clone(elem::C1D{M}) where M
  nodes = copy(elem.nodes)
  Nx    = tuple([copy(x) for x in elem.Nx]...)

  C2D{M}(nodes, Nx, elem.wgt, elem.V, elem.mat)
end
